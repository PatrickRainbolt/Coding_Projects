...
def sanitize_input(text: str) -> str:
    sanitized_text = bytearray()
    for c in text:
        if c.lower() not in CHARSET and c != SHIFT_CHAR:
            raise ValueError(f"Unsupported character: {c}")
        sanitized_text.append(ord(c))
    return sanitized_text

# Convert text with uppercase to internal format using ^ marker.
def to_internal_format(text: str) -> bytearray:
    sanitized_text = sanitize_input(text)  # Ensure only valid characters are in the input
    result = bytearray()
    for c in sanitized_text:
        char = chr(c)  # Convert byte back to character
        if char.isupper():
            result.append(ord(SHIFT_CHAR))
            result.append(ord(char.lower()))  # Store lowercase character after shift
        else:
            result.append(c)
    return result

# Convert internal format (with ^ markers) back to text with uppercase.
def from_internal_format(text: bytearray) -> str:
    result = []
    skip = False
    for i in range(len(text)):
        if skip:
            skip = False
            continue
        c = chr(text[i])  # Convert byte to character
        if c == SHIFT_CHAR and i + 1 < len(text):
            result.append(chr(text[i + 1]).upper())
            skip = True
        else:
            result.append(c)
    return ''.join(result)

# Encodes text to compact 6-bit format using bytearray.
def encode_6bit(text: str) -> bytearray:
    sanitized_text = sanitize_input(text)  # Ensure only valid characters are in the input
    bits = ''
    for c in sanitized_text:
        bits += format(char_to_index[chr(c)], '06b')  # Convert character byte to 6-bit binary

    # Pad to byte boundary
    if len(bits) % 8 != 0:
        bits += '0' * (8 - len(bits) % 8)

    # Convert the bits to bytearray
    encoded = bytearray()
...
def decode_6bit(data: bytearray) -> str:
...
print(f" Internal format: bytes[{len(internal)}]:  {internal.decode('utf-8', 'ignore')}")

# Encode the internal format to 6-bit
encoded = encode_6bit(internal.decode('utf-8', 'ignore'))
...
final_text = from_internal_format(bytearray(decoded_internal, 'utf-8'))
